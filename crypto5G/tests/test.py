...
def teste_snowv():
    ...
    out = snowv_initializer(key, iv, 0).hex()

    for i in range(16):
        for j in range(0, 32, 2):
            print(out[(32*i)+j:(32*i)+j+2], end=" ")
        print()

    print("Keystream phase, z =")
    for i in range(8):
        z = snowv_keystream().hex()
        for j in range(0, 32, 2):
            print(z[j:j+2], end=" ")
...
def snow3g_EEA1_testset_1():
    ...
    return snow.EEA1(key, count, bearer, direct, data, bitlen) == output
...
def snowv_GCM_testset_1():
    ...
    enc, mac1 = snowv.gcm_encript(key, iv, plaintxt, aad)
